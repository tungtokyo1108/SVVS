...
class DMM_VFS():
    ...
    def _estimate_alpha(self, X):
        ...
        n_samples, n_features = X.shape
        means_ = self.gamma_vi / self.delta_vi
        ...
        self.delta_vi = self.gamma_vi
        
    def _estimate_beta(self, X):
        
        means_rj = self.iota_vi / self.kappa_vi
    ...
    def _estimate_log_prob(self, X):
        ...
        sum_2_1 = means_ * (digamma(self.gamma_vi) - log_means_) * digamma(means_.sum(axis=1))[:, np.newaxis]
        ...
        sum_2_2 = digamma(sum_2_2) * np.dot(self.selected, (means_ * (digamma(self.gamma_vi) - log_means_)).T)
        ...
        for k in range(self.n_components):
            y = X + means_[k]
            y = self.selected * digamma(y) * means_[k] * (digamma(self.gamma_vi)[k] - log_means_[k])
            sum_4_1[:, k] = np.sum(y, axis=1)
            
        sum_4_2 = np.dot(self.selected, (means_ * digamma(means_) * (digamma(self.gamma_vi) - log_means_)).T)
    ...
    def _estimate_log_prob_selected(self, X):
        ...
        for k in range(self.n_components):
            y = X + means_[k]
            sum_2_2[:, k] = np.sum(y, axis=1)
        sum_2_2 = np.dot((self.resp * digamma(sum_2_2)), (means_ * (digamma(self.gamma_vi) - log_means_)))
        part_2 = sum_2_1 - sum_2_2
        ...
        for k in range(self.n_components):
            y = X + means_[k]
            y = self.resp[:,k][:, np.newaxis] * digamma(y) * means_[k] * (digamma(self.gamma_vi)[k] - log_means_[k])
            sum_4_1[k] = y
        sum_4_1 = np.sum(sum_4_1, axis=0)
        sum_4_2 = np.dot(self.resp, (means_ * digamma(means_) * (digamma(self.gamma_vi) - log_means_)))
        part_4 = sum_4_1 - sum_4_2
        
        
        estimate_log_select = part_1[:, np.newaxis] + part_2 + part_3 + part_4
    ...
    def _estimate_log_prob_rejected(self, X):
        ...
        means_ = self.iota_vi / self.kappa_vi
        log_means_ = np.log(means_)

 
        part_1 = gammaln(means_.sum()) - gammaln((X + means_).sum(axis=1))
        part_1 = part_1 / part_1.sum()
        
        
        sum_2_1 = means_ * (digamma(self.iota_vi) - log_means_) * digamma(means_.sum())
        sum_2_2 = means_ * (digamma(self.iota_vi) - log_means_) * digamma((X + means_).sum(axis=1)).sum()
        part_2 = sum_2_1 - sum_2_2
        
        
        part_3 = gammaln((X + means_)) - gammaln(means_)


        sum_4_1 = means_ * (digamma(self.iota_vi) - log_means_) * digamma((X + means_))
        sum_4_2 = means_ * (digamma(self.iota_vi) - log_means_) * digamma((means_))
        part_4 = sum_4_1 - sum_4_2
        
        
        estimate_log_reject = part_1[:, np.newaxis] + part_2 + part_3 + part_4
    ...
    def fit_predict(self, X):
        ...
        return log_resp, clus_update, prob_selected
