...
# -*- coding: utf-8 -*-
"""
Random Search Hyperparameter Optimisation for DMM_SVVS_Variational_v4
======================================================================
...
DEFAULT_MFM_DELTA_RANGE       = (0.1, 5.0)    # float, sampled log-uniformly
DEFAULT_MFM_GAMMA_RANGE       = (0.5, 10.0)   # float, sampled log-uniformly
...
def _sample_config(rng,
    ...
    mfm_delta_range       : (float_low, float_high)  — log-uniform
    mfm_gamma_range       : (float_low, float_high)  — log-uniform
    ...
    return {
        "K_max":           _sample_int(
                               ...
                               int(K_max_range[0]),
                               int(K_max_range[1])),
        "mfm_delta":       _sample_loguniform(
                               rng,
                               float(mfm_delta_range[0]),
                               float(mfm_delta_range[1])),
        "mfm_gamma":       _sample_loguniform(
                               rng,
                               float(mfm_gamma_range[0]),
                               float(mfm_gamma_range[1])),
...
def random_search(X,
                  ...
                  verbose               = True):
    """
    Random search over DMM_SVVS_Variational_v4 hyperparameters.
    ...
    mfm_delta_range       : tuple (float_low, float_high)
        Range for the MFM Dirichlet concentration δ.
        Sampled log-uniformly (small values are well-explored).
        Example: (0.1, 5.0)
    mfm_gamma_range       : tuple (float_low, float_high)
        Range for the MFM Poisson rate γ.
        Sampled log-uniformly.  Example: (0.5, 10.0)
    ...
    if verbose:
        print("\nDMM-SVVS-v4  Random Hyperparameter Search")
        print("=" * 72)
        print(f"  n_trials={n_trials},  master_seed={master_seed}")
        print(f"  K_max           : {K_max_range}")
        print(f"  mfm_delta       : {mfm_delta_range}  [log-uniform]")
        print(f"  mfm_gamma       : {mfm_gamma_range}  [log-uniform]")
        print(f"  selection_prior : {selection_prior_range}")
        print("=" * 72)
        print(f"  {'#':>4}  {'ARI':>6}  {'NMI':>6}  {'K_est':>5}  "
              f"{'sec':>5}  K_max  delta    gamma    sel")
        print("  " + "-" * 66)
    ...
    for trial in range(1, n_trials + 1):
        ...
        if verbose:
            ...
            print(f"  {trial:4d}  {result['ari']:6.3f}  {result['nmi']:6.3f}  "
                  ...
                  f"{c['K_max']:5d}  {c['mfm_delta']:7.4f}  "
                  f"{c['mfm_gamma']:7.4f}  "
                  f"{c['selection_prior']:.3f}{marker}")
    ...
    if verbose:
        ...
        if best_config is not None:
            print(f"  Best config: K_max={best_config['K_max']}  "
                  f"mfm_delta={best_config['mfm_delta']:.4f}  "
                  f"mfm_gamma={best_config['mfm_gamma']:.4f}  "
...
def refit_best(X,
    ...
    Returns
    -------
    Fitted DMM_SVVS_Variational_v4 model
    """
    ...
    for restart_idx, seed in enumerate(all_seeds):
        ...
        try:
            ...
            # Keep restart with highest ARI; tie-break by ICL proxy
