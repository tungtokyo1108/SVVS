...
# -*- coding: utf-8 -*-
"""
Random Search Hyperparameter Optimisation for DMM_SVVS_Variational_v2_3
========================================================================

Optimises five hyperparameters of DMM_SVVS_Variational_v2_3 by maximising
the Adjusted Rand Index (ARI) against ground-truth labels.

Hyperparameters searched
------------------------
  K_max            int      [low, high]         uniform integer
  py_discount      float    [0, 1)              uniform float
  py_concentration float    (0, ∞)              log-uniform float
                            (must remain > -py_discount; enforced below)
...
Usage
-----
    from random_search_dmm_svvs_v2_3 import random_search, refit_best, print_top_k
    ...
    results = random_search(
        ...
        py_concentration_range = (0.05, 5.0),
...
DEFAULT_PY_DISCOUNT_RANGE      = (0.0, 0.8)   # float, uniform;    d ∈ [0, 1)
DEFAULT_PY_CONCENTRATION_RANGE = (0.05, 5.0)  # float, log-uniform; θ > 0
...
def _sample_loguniform(rng, low, high):
    ...
    py_concentration and prune_threshold.
    """
...
def _sample_config(rng,
    ...
    Constraint enforced:
      py_concentration > -py_discount
    Since py_discount ∈ [0, 1) and py_concentration is sampled from a strictly
    positive range (default low=0.05 > 0), this is always satisfied.
    If py_concentration_range[0] <= 0 the caller must ensure validity.

    Returns
    -------
    dict with keys: K_max, py_discount, py_concentration,
                    selection_prior, prune_threshold
    """
    d   = _sample_uniform(rng, *py_discount_range)
    th  = _sample_loguniform(rng, *py_concentration_range)

    # Hard-enforce PY validity: θ must be > −d.
    # Because py_concentration_range[0] > 0 > -d by construction (d ≥ 0),
    # this is always satisfied; the clamp is a safety net for edge cases
    # where the user passes a range that includes 0.
    th = max(th, -d + 1e-6)
...
def _run_trial(X, true_labels, config, trial_seed):
    ...
    try:
        model  = DMM_SVVS_Variational_v2_3(**params)
        model.fit(X)
...
def random_search(X,
                  ...
                  verbose                  = True):
    """
    Random search over DMM_SVVS_Variational_v2_3 hyperparameters,
    ...
    py_discount_range : tuple (float_low, float_high)
        Search range for the Pitman-Yor discount d ∈ [0, 1).
        Sampled uniformly.  Example: (0.0, 0.8)
    py_concentration_range : tuple (float_low, float_high)
        Search range for the PY concentration θ > 0.
        Sampled log-uniformly so that small values (near 0) are well-explored.
        Example: (0.05, 5.0)
    ...
    if verbose:
        ...
        print(f"  py_concentration : {py_concentration_range}  [log-uniform, θ > 0]")
...
def refit_best(X,
    ...
    Returns
    -------
    Fitted DMM_SVVS_Variational_v2_3 model with the highest ARI.
    """
    X           = np.asarray(X, dtype=float)
    true_labels = np.asarray(true_labels)
    ...
    for idx, seed in enumerate(all_seeds):
        ...
        try:
            m     = DMM_SVVS_Variational_v2_3(**params)
            m.fit(X)
            pred  = m.predict(X)
...
if __name__ == "__main__":
    ...
    # ── Save all results ──────────────────────────────────────────────────
    save_results(results, path="random_search_v2_3_results.json")
