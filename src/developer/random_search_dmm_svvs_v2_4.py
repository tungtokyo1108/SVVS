...
# -*- coding: utf-8 -*-
"""
Random Search Hyperparameter Optimisation for DMM_SVVS_Variational_v2_4
========================================================================

Optimises five hyperparameters of DMM_SVVS_Variational_v2_4 by maximising
the Adjusted Rand Index (ARI) against ground-truth labels.

The only structural difference from random_search_dmm_svvs_v2_3.py is that
the two PY-specific parameters (py_discount, py_concentration) are replaced
by the two MFM-specific parameters (mfm_delta, mfm_gamma).  All sampling
strategies, output formatting, refit logic, and utility functions are
preserved unchanged.

Hyperparameters searched
------------------------
  K_max            int      [low, high]         uniform integer
  mfm_delta        float    (0, ∞)              log-uniform float
                             Symmetric Dirichlet concentration δ.
                             Log-uniform: equal probability mass per decade.
  mfm_gamma        float    (0, ∞)              log-uniform float
                             Poisson(γ)+1 rate. Small γ → fewer prior clusters.
...
Usage
-----
    from random_search_dmm_svvs_v2_4 import random_search, refit_best, print_top_k
    ...
    results = random_search(
        ...
        K_max_range           = (3, 15),
        mfm_delta_range       = (0.1, 5.0),
        mfm_gamma_range       = (0.1, 5.0),
...
DEFAULT_MFM_DELTA_RANGE       = (0.1, 5.0)   # float, log-uniform; δ > 0
DEFAULT_MFM_GAMMA_RANGE       = (0.1, 5.0)   # float, log-uniform; γ > 0
...
def _sample_loguniform(rng, low, high):
    ...
    mfm_delta, mfm_gamma, and prune_threshold.
    """
...
def _sample_config(rng,
    ...
    Both mfm_delta and mfm_gamma must be strictly positive; this is guaranteed
    by using log-uniform sampling from ranges with positive lower bounds.

    Returns
    -------
    dict with keys: K_max, mfm_delta, mfm_gamma,
                    selection_prior, prune_threshold
    """
    return {
        "K_max":           _sample_int(rng, *K_max_range),
        "mfm_delta":       _sample_loguniform(rng, *mfm_delta_range),
        "mfm_gamma":       _sample_loguniform(rng, *mfm_gamma_range),
...
def _run_trial(X, true_labels, config, trial_seed):
    ...
    try:
        model  = DMM_SVVS_Variational_v2_4(**params)
        model.fit(X)
...
def random_search(X,
                  ...
                  mfm_delta_range       = DEFAULT_MFM_DELTA_RANGE,
                  mfm_gamma_range       = DEFAULT_MFM_GAMMA_RANGE,
                  selection_prior_range = DEFAULT_SELECTION_PRIOR_RANGE,
                  prune_threshold_range = DEFAULT_PRUNE_THRESHOLD_RANGE,
                  master_seed           = 42,
                  verbose               = True):
    """
    Random search over DMM_SVVS_Variational_v2_4 hyperparameters,
    ...
    K_max_range : tuple (int_low, int_high)
        ...
        Example: (3, 15)
    mfm_delta_range : tuple (float_low, float_high)
        Search range for the MFM symmetric Dirichlet concentration δ > 0.
        Sampled log-uniformly so that small values are well-explored.
        Larger δ → more uniform mixing weights → more clusters survive.
        Example: (0.1, 5.0)
    mfm_gamma_range : tuple (float_low, float_high)
        Search range for the Poisson(γ)+1 prior rate γ > 0.
        Sampled log-uniformly.  Small γ → prior mean of ~1+γ clusters.
        Example: (0.1, 5.0)
    ...
    # ── Header ────────────────────────────────────────────────────────────
    if verbose:
        print("\nDMM-SVVS v2.4 — Random Hyperparameter Search  (scored by ARI)")
        print("=" * 90)
        print(f"  n_trials      = {n_trials},   master_seed = {master_seed}")
        print(f"  K_max            : {K_max_range}   [uniform int]")
        print(f"  mfm_delta        : {mfm_delta_range}   [log-uniform, δ > 0]")
        print(f"  mfm_gamma        : {mfm_gamma_range}   [log-uniform, γ > 0]")
...
def refit_best(X,
    ...
    Returns
    -------
    Fitted DMM_SVVS_Variational_v2_4 model with the highest ARI.
    """
    X           = np.asarray(X, dtype=float)
    true_labels = np.asarray(true_labels)
    ...
    for idx, seed in enumerate(all_seeds):
        ...
        try:
            m     = DMM_SVVS_Variational_v2_4(**params)
            m.fit(X)
            pred  = m.predict(X)
...
def _print_row_header():
    print(f"  {'#':>5}  {'ARI':>7}  {'NMI':>7}  {'K_est':>5}  {'sec':>5}  "
          f"{'K_max':>5}  {'δ':>8}  {'γ':>8}  {'sel':>6}  {'prune':>8}  {'seed':>12}")
    print("  " + "-" * 90)
...
def save_results(search_result, path="random_search_v2_4_results.json"):
    """
...
if __name__ == "__main__":
    ...
    # ── Save all results ──────────────────────────────────────────────────
    save_results(results, path="random_search_v2_4_results.json")
